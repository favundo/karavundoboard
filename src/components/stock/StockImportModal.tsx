...
const StockImportModal = ({ open, onClose }: StockImportModalProps) => {
  ...
  const appendStock = useAppendStockInventory();
  ...
  const handleConfirm = async (e: React.MouseEvent) => {
    ...
    try {
      await appendStock.mutateAsync(parseResult.items);
      setStep("success");
    } catch (err) {
  ...
  const PREVIEW_COLS: { key: keyof InventoryItem; label: string }[] = [
    ...
    { key: "dns", label: "DNS" },
    { key: "warranty_end_date", label: "Fin garantie" },
  ];
      ...
      <div className="relative z-10 w-full max-w-4xl rounded-2xl border border-border bg-card shadow-2xl">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-foreground">Mettre à jour le stock</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-border bg-muted/20 p-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {["Nom", "Asset", "Type", "SN", "DNS", "UID", "Absence", "Remarques", "App. ESET", "Fin de garantie", "Durée garantie"].map((col) => (
            ...
            <div className="space-y-4">
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-border bg-muted/20">
                        {PREVIEW_COLS.map((c) => (
                          <th key={c.key} className="px-3 py-2 text-left font-semibold uppercase tracking-wider text-muted-foreground">
                            {c.label}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {parseResult.items.slice(0, 8).map((item, i) => (
                        <tr key={i} className="border-b border-border/50 hover:bg-muted/20">
                          ...
                          <td className="max-w-[140px] truncate px-3 py-2 font-mono text-[10px] text-muted-foreground">
                            {item.dns || "—"}
                          </td>
                          <td className="px-3 py-2 text-muted-foreground">{item.warranty_end_date || "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="rounded-xl border border-primary/20 bg-primary/5 px-4 py-3">
                <p className="text-xs text-primary">
                  ✅ Ces {parseResult.items.length} équipements seront <strong>ajoutés ou mis à jour</strong> dans le stock, sans supprimer les données actuelles.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="flex flex-col items-center gap-4 py-8 text-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {parseResult?.items.length} équipements importés — le stock est mis à jour.
                </p>
              </div>
            </div>
          )}
          ...
          {step === "preview" && (
            <>
              ...
              <button
                ...
                disabled={appendStock.isPending}
