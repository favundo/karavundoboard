...
interface FormData {
  nom: string; uid: string; matricule: string; pseudo: string; service: string;
  type: string; sn: string; dns: string; windows_version: string; remarques: string;
}

interface Props { open: boolean; onClose: () => void; }

const AbcroisiereAffecterModal = ({ open, onClose }: Props) => {
  const [step, setStep] = useState<Step>("asset");
  const [asset, setAsset] = useState("");
  const [alreadyAssigned, setAlreadyAssigned] = useState<string | null>(null);
  const [form, setForm] = useState<FormData>({
    nom: "", uid: "", matricule: "", pseudo: "", service: "", type: "", sn: "", dns: "", windows_version: "", remarques: "",
  });
  ...
  const lookupMutation = useMutation({
    mutationFn: async (assetCode: string) => {
      const { data, error } = await supabase
        ...
        .eq("asset", assetCode.trim()).maybeSingle();
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("abcroisiere_inventory").update({
        nom: form.nom.trim(), uid: form.uid.trim(), matricule: form.matricule.trim(),
        pseudo: form.pseudo.trim(), service: form.service.trim(), type: form.type,
        sn: form.sn.trim(), dns: form.dns.trim(), windows_version: form.windows_version.trim(),
  ...
  const handleClose = () => {
    setStep("asset"); setAsset(""); setAlreadyAssigned(null);
    setForm({ nom: "", uid: "", matricule: "", pseudo: "", service: "", type: "", sn: "", dns: "", windows_version: "", remarques: "" });
    lookupMutation.reset(); onClose();
  };

  const handleSearch = async () => {
    if (!asset.trim()) return;
    const result = await lookupMutation.mutateAsync(asset.trim());
    if (!result) { toast.error(`Aucun équipement trouvé avec l'asset "${asset.trim()}"`); return; }
    if (result.nom && result.nom.trim() !== "") {
      setAlreadyAssigned(result.uid ? `${result.nom} (${result.uid})` : result.nom);
    } else { setAlreadyAssigned(null); }
    setForm({
      nom: result.nom ?? "", uid: result.uid ?? "", matricule: result.matricule ?? "",
      pseudo: result.pseudo ?? "", service: result.service ?? "", type: result.type ?? "",
      sn: result.sn ?? "", dns: result.dns ?? "", windows_version: result.windows_version ?? "",
        ...
        {step === "asset" && (
          <>
            <DialogHeader>
              ...
              <DialogDescription>Saisissez le numéro d'asset du PC à affecter.</DialogDescription>
            </DialogHeader>
          ...
          <>
            ...
            {alreadyAssigned && (
              <div className="flex items-start gap-2 rounded-lg border border-orange-500/30 bg-orange-500/10 p-3 text-sm text-orange-600 dark:text-orange-400">
                ...
                <span>Cet asset est déjà affecté à <span className="font-semibold">{alreadyAssigned}</span>. Vous pouvez tout de même modifier l'affectation.</span>
              </div>
            )}
            <div className="grid grid-cols-2 gap-3 py-1 max-h-[60vh] overflow-y-auto pr-1">
              ...
              <div className="space-y-1">
                <Label>Service</Label>
                <select value={form.service} onChange={set("service")} className={SC}>
              ...
              <div className="space-y-1">
                <Label>Type</Label>
                <select value={form.type} onChange={set("type")} className={SC}>
                  <option value="">— Choisir —</option>
              ...
              <div className="col-span-2 space-y-1">
                ...
                <select value={form.windows_version} onChange={set("windows_version")} className={SC}>
                  <option value="">— Choisir —</option>
                  ...
                  <option>Linux</option><option>Autres</option>
                </select>
              </div>
              <div className="col-span-2 space-y-1">
                <Label>Remarques</Label>
                <textarea value={form.remarques} onChange={set("remarques")} rows={2}
            ...
            <DialogFooter>
              ...
              <Button onClick={() => saveMutation.mutate()} disabled={!form.nom.trim() || !form.uid.trim() || saveMutation.isPending} className="bg-violet-600 hover:bg-violet-700 text-white">
                <UserCheck size={14} />{saveMutation.isPending ? "Enregistrement…" : "Affecter"}
              </Button>
            </DialogFooter>
          </>
        )}
...
export default AbcroisiereAffecterModal;
