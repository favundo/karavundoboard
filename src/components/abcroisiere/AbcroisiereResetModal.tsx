...
interface AbcroisiereResetModalProps {
  open: boolean;
  onClose: () => void;
}

const AbcroisiereResetModal = ({ open, onClose }: AbcroisiereResetModalProps) => {
  ...
  const handleReset = async () => {
    setLoading(true);
    await supabase.from("abcroisiere_inventory").delete().neq("id", "00000000-0000-0000-0000-000000000000");
            ...
            <>
              <div className="flex items-start gap-3 rounded-xl border border-destructive/20 bg-destructive/5 p-4">
                ...
                <p className="text-xs text-destructive">
                  Cette action supprimera <strong>définitivement</strong> tous les équipements de l'onglet <strong>ABcroisière</strong>. Cette opération est irréversible.
                </p>
              </div>
...
export default AbcroisiereResetModal;
